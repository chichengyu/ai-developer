...
Usage:
    python analyze_script.py <project_dir> [--script 02_script.json]
      [--output-json 02_script_analysis.json] [--output-md 02_script_analysis.md]
      [--dry-run]

Reads:
  <project_dir>/02_script.json or 02_script.md
  <project_dir>/02_character_analysis.md
  <project_dir>/character-bible.md
  <project_dir>/scene-bible.md
  <project_dir>/00_meta.json

Writes:
  <project_dir>/02_script_analysis.json
  <project_dir>/02_script_analysis.md
...
def load_script(project: Path, script_path: Optional[Path]) -> Optional[Dict[str, Any]]:
    if script_path is not None:
        ...
        json_path = project / "02_script.json"
        md_path = project / "02_script.md"
        path = json_path if json_path.exists() else md_path
    if not path.exists():
        return None
...
def build_analysis(project: Path, script: Dict[str, Any]) -> Dict[str, Any]:
    meta = load_json(project / "00_meta.json") or {}
    scenes = script.get("scenes", [])
    bible = parse_character_bible(project / "character-bible.md")
    analysis = parse_character_analysis(project / "02_character_analysis.md")
    scene_bibles = parse_scene_bible(project / "scene-bible.md")
...
def main(argv: List[str]) -> int:
    ...
    output_json = args.output_json or project / "02_script_analysis.json"
    output_md = args.output_md or project / "02_script_analysis.md"
    if args.dry_run:
