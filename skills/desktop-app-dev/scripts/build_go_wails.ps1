...
param(
    ...
    [string] $Arch = "x64",
    [switch] $Nsis,                  # build NSIS installer (default: true)
    [switch] $Clean,
...
$exe = "build/bin/myapp.exe"
if (-not (Test-Path $exe)) { Write-Host "==> Looking for built EXE in build/bin/ ..." -ForegroundColor Yellow; Get-ChildItem build/bin/ }
$nsis = "build/bin/myapp-amd64-installer.exe"
if (Test-Path $nsis) { Write-Host "==> NSIS installer: $nsis" -ForegroundColor Green }
