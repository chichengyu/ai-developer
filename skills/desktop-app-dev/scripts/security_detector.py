...
_CLOUDFLARE_CHALLENGE_RE = re.compile(
    ...
    r"enable javascript and cookies to continue|cf-chl|challenge-platform|"
...
class SecurityReport:
    ...
    def needs_captcha(self) -> bool:
        return any(item.kind in {CAPTCHA_REQUIRED, CLOUDFLARE_CHALLENGE} for item in self.findings)
...
def detect_security_mechanisms(
    ...
    *,
    ...
    if _CLOUDFLARE_CHALLENGE_RE.search(text) or mitigated.lower() == "challenge":
    ...
    if _WAF_BLOCKED_RE.search(text) and not any(
        item.kind in {CLOUDFLARE_BLOCKED, CLOUDFLARE_CHALLENGE} for item in findings
    ):
        _add(
            findings,
            WAF_BLOCKED,
    ...
    if _RATE_LIMIT_RE.search(text) and status in {None, 200, 403, 429, 500, 502, 503, 504}:
        _add(
            findings,
            RATE_LIMITED,
            0.85,
    ...
    if _LOGIN_REQUIRED_RE.search(text) and not any(
        item.kind == LOGIN_REQUIRED for item in findings
    ):
        _add(
            findings,
            LOGIN_REQUIRED,
