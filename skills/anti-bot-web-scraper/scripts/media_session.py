...
class MediaSession:
    ...
    def __init__(
        ...
        *,
        ...
        self._current_proxy_used = self.proxy
        self._pinned_proxy: str | None = None
    ...
    def _current_proxy(self) -> str | None:
        if self._pinned_proxy is not None:
            return normalize_proxy(self._pinned_proxy)
        if self.proxy_pool is not None:
            if any(
                ...
                return normalize_proxy(
                    self.proxy_pool.get_proxy_for(
                        region=self.proxy_region,
                        country=self.proxy_country,
                        city=self.proxy_city,
                        provider=self.proxy_provider,
                    )
                )
            return normalize_proxy(self.proxy_pool.get_proxy())
        return self.proxy

    def pin_proxy(self, proxy: str | None) -> None:
        """Pin one proxy for a logical request so cookies stay valid."""
        self._pinned_proxy = normalize_proxy(proxy)

    def clear_pinned_proxy(self) -> None:
    ...
    def _report_proxy_failure(self, proxy: str | None) -> None:
        ...
        if proxy is not None and self._pinned_proxy == proxy:
            self._pinned_proxy = None
